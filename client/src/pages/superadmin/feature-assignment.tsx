...
export default function FeatureAssignment() {
  ...
  const { data: schoolFeatures = [], isLoading: schoolFeaturesLoading, refetch: refetchSchoolFeatures } = useQuery({
  ...
  const toggleFeatureMutation = useMutation({
    mutationFn: async ({ schoolId, featureId, enabled }: { schoolId: string; featureId: string; enabled: boolean }) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/superadmin/schools", selectedSchool, "features"] });
      toast({
        title: "Success",
  ...
  const assignFeaturesMutation = useMutation({
    mutationFn: async ({ schoolId, featureIds }: { schoolId: string; featureIds: string[] }) => {
  ...
  const getFeatureStatus = (featureId: string) => {
    const schoolFeature = schoolFeatures.find((sf: SchoolFeature) => sf.featureId === featureId);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="feature-assignment-page">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900" data-testid="page-title">Feature Assignment</h1>
      ...
      <Tabs defaultValue="individual" className="space-y-6">
        <TabsList>
          <TabsTrigger value="individual" data-testid="tab-individual">Individual Assignment</TabsTrigger>
          <TabsTrigger value="bulk" data-testid="tab-bulk">Bulk Assignment</TabsTrigger>
        </TabsList>
                ...
                {selectedSchool ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600">
                      Please select a school from the list to manage its feature assignments.
                    </p>
                  </div>
                )}
              </CardContent>
